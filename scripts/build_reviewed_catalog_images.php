...
$dataFile = database_path("data/reviewed-{$dataset}-images.php");
if (! is_file($dataFile)) {
    throw new RuntimeException("Reviewed image dataset not found: {$dataset}");
}

$images = require $dataFile;
$outputRoot = public_path("images/catalog-reviewed/{$dataset}-sourced");
$temporaryRoot = sys_get_temp_dir().DIRECTORY_SEPARATOR."masterscule-reviewed-{$dataset}";
@mkdir($temporaryRoot, 0777, true);

function squareCanvas(GdImage $source, int $size): GdImage
{
    $canvas = imagecreatetruecolor($size, $size);
    $white = imagecolorallocate($canvas, 255, 255, 255);
    imagefill($canvas, 0, 0, $white);
    $scale = min(($size * 0.84) / imagesx($source), ($size * 0.84) / imagesy($source));
...
foreach ($images as $sku => $source) {
    $url = is_array($source) ? $source['image_url'] : $source;
    $slug = Str::slug($sku);
    $temporaryFile = $temporaryRoot.DIRECTORY_SEPARATOR.$slug.'.source';
    if (! is_file($temporaryFile) || filesize($temporaryFile) === 0) {
        $process = proc_open(
            ['curl.exe', '-L', '--fail', '--silent', '--show-error', '--max-time', '45', '--output', $temporaryFile, $url],
            [1 => ['pipe', 'w'], 2 => ['pipe', 'w']],
            $pipes,
        );
    ...
    if (! $source instanceof GdImage || imagesx($source) < 180 || imagesy($source) < 180) {
        $skipped[$sku] = 'Source is smaller than 180 px on one side or is not decodable.';
    ...
    $main = squareCanvas($source, 1200);
    $watermark->apply($main);
    $preview = squareCanvas($source, 600);
    $thumb = squareCanvas($source, 300);
    imagedestroy($source);
    ...
    $results[$sku] = ['width' => 1200, 'height' => 1200, 'source' => $url];
}
