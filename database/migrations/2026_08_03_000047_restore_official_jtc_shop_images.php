...
return new class extends Migration
{
    ...
    {
        $images = require database_path('data/reviewed-jtc-shop-images.php');

        DB::transaction(function () use ($images): void {
            foreach ($images as $sku => $source) {
                $slug = Str::slug($sku);
                $directory = '/images/catalog-reviewed/jtc-shop-sourced/'.$slug;
                ...
                DB::table('products')->where('id', $product->id)->update([
                    ...
                    'source_domain' => 'jtcautotools.com',
                ...
                if ($product->source_parser_item_id) {
                    ...
                    DB::table('product_parser_items')->where('id', $product->source_parser_item_id)->update([
                        ...
                        'official_source_domain' => 'jtcautotools.com',
                    ...
                    DB::table('product_parser_sources')->updateOrInsert(
                        ...
                        [
                            'domain' => 'jtcautotools.com',
                            'title' => 'Official JTC product card '.$sku,
                            'snippet' => 'Exact JTC product card and image verified in the manufacturer store.',
                            'source_type' => 'official_manufacturer',
                            'confidence_score' => 100,
                            'raw_data_json' => json_encode(['sku' => $sku, 'verification' => 'official_store_product'], JSON_UNESCAPED_SLASHES),
                            'created_at' => $now,
                            'updated_at' => $now,
                        ]
                    );
