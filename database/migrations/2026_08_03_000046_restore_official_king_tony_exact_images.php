...
return new class extends Migration
{
    ...
    {
        $images = require database_path('data/reviewed-king-tony-exact-images.php');

        DB::transaction(function () use ($images): void {
            foreach ($images as $sku => $sourceUrl) {
                $slug = Str::slug($sku);
                $directory = '/images/catalog-reviewed/king-tony-exact-sourced/'.$slug;
                ...
                if ($product->source_parser_item_id) {
                    ...
                    DB::table('product_parser_items')->where('id', $product->source_parser_item_id)->update([
                        ...
                        'image_source_type' => 'official_manufacturer_exact_sku',
                        'needs_image_review' => false,
