...
return new class extends Migration
{
    ...
    {
        $images = require database_path('data/reviewed-gys-legacy-images.php');

        DB::transaction(function () use ($images): void {
            foreach ($images as $sku => $source) {
                $slug = Str::slug($sku);
                $directory = '/images/catalog-reviewed/gys-legacy-sourced/'.$slug;
                ...
                if ($product->source_parser_item_id) {
                    ...
                    DB::table('product_parser_items')->where('id', $product->source_parser_item_id)->update([
                        ...
                        'image_source_type' => 'official_manufacturer_archive',
                        'needs_image_review' => false,
